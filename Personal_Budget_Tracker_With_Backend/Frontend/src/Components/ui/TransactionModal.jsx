...
const TransactionModal = ({ isOpen, onClose, transactionToEdit = null }) => {
  ...
  // submitting : empêche le double-clic et affiche un état de chargement
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await modifierTransaction(transactionToEdit.id, payload);
      } else {
        await ajouterTransaction(payload);
      }
      onClose();
    } catch (err) {
      // Affiche l'erreur API dans le formulaire si la requête échoue
      setErrors({ api: err.message || 'Une erreur est survenue' });
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-600 disabled:opacity-60"
            >
              {submitting && (
          ...
          {/* Erreur API */}
          {errors.api && (
            <p className="text-center text-xs text-red-500">{errors.api}</p>
          )}
        </form>
      </div>
    </div>
