...
import { searchAndFilterRecipes } from "../../utils/localStorage";
...
export default function RecipesPage() {
  ...
  useEffect(() => {
    setLoading(true);
    const t = setTimeout(() => {
      setRecipes(searchAndFilterRecipes({ query, category: selCategory, prepTime, difficulty, diet: selDiet, sortBy }));
      setPage(1);
      setLoading(false);
    }, 180);
    return () => clearTimeout(t);
  }, [query, selCategory, prepTime, difficulty, selDiet, sortBy]);

  const totalPages = Math.ceil(recipes.length / PER_PAGE);
  const paginated = recipes.slice((page - 1) * PER_PAGE, page * PER_PAGE);
        ...
        <FilterGroup title="Catégorie">
          {CATEGORIES.map(cat => (
            <RadioItem key={cat} label={cat} checked={selCategory === cat}
              onChange={() => setSelCategory(selCategory === cat ? "" : cat)} name="cat" />
          ))}
        </FilterGroup>
        ...
        <FilterGroup title="Temps de préparation">
          {PREP_TIMES.map(t => (
            <RadioItem key={t} label={t} checked={prepTime === t}
              onChange={() => setPrepTime(t)} name="time" />
          ))}
        </FilterGroup>
        ...
        <FilterGroup title="Régimes">
          {DIETS.map(d => (
            <RadioItem key={d} label={d} checked={selDiet === d}
              onChange={() => setSelDiet(selDiet === d ? "" : d)} name="diet" />
          ))}
        </FilterGroup>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              ...
              <span style={{ fontSize: "0.875rem", color: "#6b7280" }}>
                {loading ? "Chargement..." : `Affichage de ${recipes.length} recette${recipes.length !== 1 ? "s" : ""}`}
              </span>
            </div>
