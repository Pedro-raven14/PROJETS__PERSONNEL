...
import {
  getRecipeById, getUserById, getCommentsByRecipe, addComment,
  deleteComment, toggleFavorite, rateRecipe, getUserRating,
  deleteRecipe, searchAndFilterRecipes,
...
export default function RecipeDetail() {
  ...
  useEffect(() => {
    ...
    setRecipe(r);
    setAuthor(getUserById(r.authorId));
    setComments(getCommentsByRecipe(id));
    setIsFav((r.favorites || []).includes(currentUser?.id));
    if (currentUser) setUserRating(getUserRating(id, currentUser.id));
    const all = searchAndFilterRecipes({ category: r.category });
    setRelated(all.filter(x => x.id !== id).slice(0, 3));
  }, [id, currentUser, navigate]);

  function handleFavorite() {
    if (!currentUser) { navigate("/connexion"); return; }
    const s = toggleFavorite(id, currentUser.id);
    setIsFav(s);
    addToast(s ? "Ajouté aux favoris ❤️" : "Retiré des favoris");
    setRecipe(getRecipeById(id));
  }

  function handleRate(rating) {
    if (!currentUser) { navigate("/connexion"); return; }
    rateRecipe(id, currentUser.id, rating);
  ...
  async function handleComment(e) {
    ...
    setSubmitting(true);
    addComment(id, currentUser.id, commentText);
    setComments(getCommentsByRecipe(id));
    setCommentText("");
    setSubmitting(false);
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "2rem 1.5rem" }}>
      ...
      <div style={{
        ...
        {recipe.imageBase64 ? (
          <img src={recipe.imageBase64} alt={recipe.title}
            ...
            {comments.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                {comments.map(comment => {
                  const ca = getUserById(comment.authorId);
                  return (
