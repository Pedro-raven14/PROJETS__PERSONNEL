...
type Employee = {
  ...
  soldeConges: number; mustChangePassword: boolean;
  role: { nom: string };
  permissions: { nom: string }[];
  equipe?: { equipeId: number; nom: string; departement?: { nom: string } } | null;
};
type Objectif = { objectifId: number; titre: string; status: string; points: number; date_debut: string; date_fin: string; };
type Conge = { congeId: number; date_debut: string; date_fin: string; statut: string; commentaire?: string; typeConge: { nomType: string }; };
type Evaluation = { evaluationId: number; note_globale: number; date: string; commentaire?: string; cycle: { nom: string }; evaluateur?: { prenom: string; nom: string }; };
type Formation = { formationId: number; titre: string; description?: string; date_debut: string; date_fin: string; duree: number; };
type Contrat = { contratId: number; type: string; date_debut: string; date_fin?: string; poste: string; salaire: number; statut: string; signe: boolean; };
type EmployeCompetence = { id: number; niveau: number; competence: { competenceId: number; nom: string; categorie?: string }; };
...
const Profil = () => {
  ...
  if (!employee) return <p style={{ padding: '2rem', textAlign: 'center', color: 'var(--color-muted-foreground)' }}>Profil introuvable.</p>;
  ...
  const evalChartData = [...evals]
    ...
    .map((e) => ({ cycle: e.cycle?.nom ?? e.date, note: parseFloat(String(e.note_globale)) || 0 }));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', gap: '1.5rem' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginTop: '0.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                <Mail style={{ width: '14px', height: '14px' }} />{employee.email}
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                <Phone style={{ width: '14px', height: '14px' }} />{employee.phone}
              </span>
            </div>
          </div>
          {/* Solde congés — se place en bas sur mobile, à droite sur desktop */}
          <div style={{
      ...
      <Tabs defaultValue="general">
        <TabsList style={{ flexWrap: 'wrap', height: 'auto' }}>
          ...
          <TabsTrigger value="conges">Congés {conges.length > 0 && `(${conges.length})`}</TabsTrigger>
          <TabsTrigger value="evaluations">Évaluations {evals.length > 0 && `(${evals.length})`}</TabsTrigger>
          <TabsTrigger value="formations">Formations {formations.length > 0 && `(${formations.length})`}</TabsTrigger>
          <TabsTrigger value="contrats">Contrats {contrats.length > 0 && `(${contrats.length})`}</TabsTrigger>
          <TabsTrigger value="competences">Compétences {competences.length > 0 && `(${competences.length})`}</TabsTrigger>
          {employee.equipe && (
            <TabsTrigger value="objectifs">Objectifs équipe {objectifs.length > 0 && `(${objectifs.length})`}</TabsTrigger>
          )}
        </TabsList>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
            ...
            <div className="stat-card" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              {employee.equipe ? (
                ...
                <p style={{ color: 'var(--color-muted-foreground)', fontSize: '0.875rem' }}>Non affecté à une équipe</p>
              )}
            </div>
          </div>
        </TabsContent>
                ...
                {conges.length === 0 ? (
                  ...
                  <div key={c.congeId} style={{ padding: '0.875rem 1rem', borderBottom: i < conges.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
                      ...
                      <span className={c.statut === 'APPROUVE' ? 'badge-status badge-success' : c.statut === 'REFUSE' ? 'badge-status badge-destructive' : 'badge-status badge-warning'}>{c.statut}</span>
                    </div>
                ...
                <tbody>
                  {conges.length === 0 ? (
                    ...
                    <tr key={c.congeId} style={{ borderBottom: '1px solid var(--color-border)' }}>
                      ...
                      <td style={{ padding: '0.75rem 1.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>{new Date(c.date_debut).toLocaleDateString('fr-FR')}</td>
                      <td style={{ padding: '0.75rem 1.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>{new Date(c.date_fin).toLocaleDateString('fr-FR')}</td>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {evals.length === 0 ? (
                    ...
                    <div key={ev.evaluationId} style={{ padding: '0.875rem 1rem', borderBottom: i < evals.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
                      ...
                      <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--color-muted-foreground)' }}>
                        {new Date(ev.date).toLocaleDateString('fr-FR')}{ev.commentaire ? ` · ${ev.commentaire}` : ''}
                      </p>
                    </div>
                  ))}
                </div>
                  ...
                  <tbody>
                    {evals.length === 0 ? (
                      ...
                      <tr key={ev.evaluationId} style={{ borderBottom: '1px solid var(--color-border)' }}>
                        ...
                        <td style={{ padding: '0.75rem 1.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>{new Date(ev.date).toLocaleDateString('fr-FR')}</td>
                        <td style={{ padding: '0.75rem 1.5rem' }}>
                          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--color-accent)' }}>{parseFloat(String(ev.note_globale)).toFixed(1)}</span>
                          <span style={{ fontSize: '0.75rem', color: 'var(--color-muted-foreground)' }}>/5</span>
                        </td>
                        <td style={{ padding: '0.75rem 1.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>{ev.commentaire || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <tbody>
                  {formations.length === 0 ? (
                    ...
                    <tr key={f.formationId} style={{ borderBottom: '1px solid var(--color-border)' }}>
                      ...
                      <td style={{ padding: '0.75rem 1.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>{new Date(f.date_debut).toLocaleDateString('fr-FR')}</td>
                      <td style={{ padding: '0.75rem 1.5rem', fontSize: '0.875rem', color: 'var(--color-muted-foreground)' }}>{new Date(f.date_fin).toLocaleDateString('fr-FR')}</td>
        ...
        {/* Onglet Contrats — salaire visible pour Admin/RH/Manager */}
        <TabsContent value="contrats">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {contrats.length === 0 ? (
              <div className="stat-card" style={{ textAlign: 'center', color: 'var(--color-muted-foreground)' }}>Aucun contrat enregistré</div>
            ) : contrats.map((c) => (
              <div key={c.contratId} className="stat-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    {c.salaire > 0 && (
                      <p style={{ margin: '0.25rem 0 0', fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-primary)' }}>
                        {Number(c.salaire).toLocaleString('fr-FR')} FCFA / mois
                      </p>
                    )}
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <span className={`badge-status ${c.statut === 'ACTIF' ? 'badge-success' : 'badge-muted'}`}>{c.statut}</span>
                    <span className={`badge-status ${c.signe ? 'badge-success' : 'badge-warning'}`}>{c.signe ? 'Signé' : 'Non signé'}</span>
                  </div>
                </div>
              </div>
            ))}
              ...
              {competences.map((ec) => {
                ...
                return (
                  <div key={ec.id} className="stat-card" style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '0.9375rem' }}>{ec.competence.nom}</span>
                      {ec.competence.categorie && (
                        <span style={{ fontSize: '0.7rem', padding: '0.1rem 0.4rem', borderRadius: '999px', backgroundColor: 'var(--color-muted)', color: 'var(--color-muted-foreground)' }}>
                          {ec.competence.categorie}
                        </span>
                      )}
                    </div>
                    <div>
                      ...
                      <div style={{ height: '8px', borderRadius: '999px', backgroundColor: 'var(--color-border)', overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: '999px', width: `${info.pct}%`, backgroundColor: info.color, transition: 'width 0.4s ease' }} />
                      </div>
