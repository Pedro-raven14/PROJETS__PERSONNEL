...
import { createRecipe, updateRecipe, getRecipeById, uploadImage } from "../../utils/api";
...
export default function CreateRecipe() {
  ...
  const [form, setForm] = useState({
    ...
    imageUrl: null, tags: "", // imageUrl (URL Cloudinary) remplace imageBase64
  });
  const [errors, setErrors] = useState({});
  const [submitting, setSubmitting] = useState(false);
  const [imagePreview, setImagePreview] = useState(null);
  const [imageFile, setImageFile] = useState(null); // Fichier brut pour l'upload
  ...
  function handleImageChange(e) {
    ...
    // Stocker le fichier brut pour l'upload lors de la soumission
    setImageFile(file);
  }
  ...
  async function handleSubmit(status = "published") {
    ...
    setSubmitting(true);
    try {
      // 1. Si une image a été sélectionnée, l'uploader d'abord sur Cloudinary
      let imageUrl = form.imageUrl;
      if (imageFile) {
        addToast("Upload de l'image en cours...", "info");
      ...
      if (isEdit) {
        ...
        const nr = await createRecipe(data);
        ...
        <Section title="Photo de la recette">
          {imagePreview ? (
            <div style={{ position: "relative" }}>
              ...
              <button type="button" onClick={() => { setImagePreview(null); setImageFile(null); setField("imageUrl", null); }}
                style={{
