...
export default function Profile() {
  ...
  useEffect(() => {
    // Charger le profil public depuis l'API
    getUserByUsernameApi(username)
      .then((user) => {
        setProfileUser(user);
        // Charger les recettes de cet utilisateur
        return getRecipesByUser(user.id);
      })
      .then((recipeList) => {
    ...
    // Si c'est le profil de l'utilisateur connecté, charger aussi ses favoris
  ...
  function handleAvatarChange(e) {
    ...
    reader.onload = ev => setSettingsForm(p => ({ ...p, avatar: ev.target.result, _avatarFile: file }));
  ...
  async function handleSaveSettings(e) {
    ...
    try {
      ...
      if (settingsForm._avatarFile) {
        const result = await uploadImage(settingsForm._avatarFile);
      ...
      addToast(err.response?.data?.message || err.message || "Erreur", "error");
    }
  }
              ...
              <div style={{
                ...
                {favorites.map(recipe => (
                  <RecipeCard key={recipe.id} recipe={recipe}
                    onFavoriteChange={() => getMyFavorites().then(setFavorites)} />
                ))}
              </div>
            )}
          </div>
