...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.username.trim()) errs.username = "Le nom d'utilisateur est requis.";
      ...
      errs.username = "3-20 caractères sans accent (Pedro1 ✓, Pédro1 ✗), chiffres ou _";
    ...
    setLoading(true);
    try {
      await signup({ fullName: form.fullName.trim(), username: form.username.trim(), email: form.email.trim(), password: form.password });
      addToast("Compte créé ! Bienvenue 🎉");
      navigate("/");
    } catch (err) {
      setErrors({ general: err.response?.data?.message || err.message || "Erreur lors de l'inscription" });
