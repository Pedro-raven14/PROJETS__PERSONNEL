...
import AjoutFormation from "../../Admin/Formations/AjoutFormation";
...
const Formations = () => {
  ...
  const fetchRecommandations = () => {
    setLoadingReco(true); setRecoError(false);
    try {
      ...
      const recos: Recommandation[] = nonInscrites.slice(0, 5).map((f, i) => ({
        rang: i + 1, formationId: f.formationId, formation: f.titre,
        niveau: f.niveau, score: Math.round(60 + Math.random() * 40), competences_cibles: [],
      }));
      setRecommandations(recos); setFormationsMap(map);
    } catch { setRecoError(true); } finally { setLoadingReco(false); }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <FormationsBase
        ...
        peutCreer={true}
        visionGlobale={true}
        AjoutFormationComponent={AjoutFormation}
      />
