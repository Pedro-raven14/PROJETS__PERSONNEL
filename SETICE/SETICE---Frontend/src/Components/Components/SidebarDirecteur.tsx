...
const SidebarDirecteur = ({ isOpen = true, onClose }: SidebarDirecteurProps) => {
  const menu = [
    {
  ...
  return (
    <>
      ...
      `}>
        ...
        <nav className="space-y-1 pt-8 flex flex-col flex-1 bg-gray-100 overflow-auto">
          {menu.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
